...
div_storybox[8].addEventListener('mouseover', ()=>{
    ...
    storyP[8].innerHTML = "<p>교태전(交泰殿)은 강녕전과 함께 왕과 왕비가 일상생활을 하던 침전이자, 왕비의 생활공간이다. ‘교태’는 ‘천지, 음양이 잘 어울려 태평을 이루다’라는 뜻이다. 교태전은 궁궐의 가장 가운데에 위치하고 있어, 왕비의 생활공간이기 때문에 중궁전이라고도 부른다.</p>";
});
...
div_storybox[13].addEventListener('mouseover', ()=>{
    ...
    storyP[13].innerHTML = "<p>건춘문(建春門)은 경복궁의 동문으로, ‘건춘’의 뜻은 ‘봄을 세우다’라는 뜻이다. 건춘문은 주로 왕세자와 동궁 영역이 위치한 관청에서 일하는 신하들이 이용하였다. 현재의 문은 고종 대에 경복궁을 다시 지을 때 지은 문이다.</p>";
});
...
div_storybox[19].addEventListener('mouseover', ()=>{
    ...
    storyP[19].innerHTML = "<p>풍기대(風旗臺)는 바람의 방향과 세기를 재는 깃발을 세운 받침대이다. 깃대 끝에 좁고 긴 깃발을 달아 그것이 날리는 방향으로 바람의 방향을 재고 흔들리는 정도로 바람의 세기를 잴 수 있었다. 풍기대는 1985년 보물로 지정되었다.</p>";
});
